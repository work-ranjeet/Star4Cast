...
using Star4Cast.ViewModels.Account;
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> SignUp(SignUpViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    // Send an email with this link  // For more information  http://go.microsoft.com/fwlink/?LinkID=532713
        ...
        [HttpGet]
        [AllowAnonymous]
        public IActionResult SignUpCasting(string returnUrl = null)
        {
            ViewData["ReturnUrl"] = returnUrl;
            return View();
        }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> SignUpCasting(SignUpCastingViewModel model, string returnUrl = null)
        {
            ViewData["ReturnUrl"] = returnUrl;
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser
                {
                    ...
                    OrgWebsite = model.OrganizationWeb,
                    OrgDesiganation = model.Desiganation
                };
                var result = await _userManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
